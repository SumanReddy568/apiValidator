...
export async function getValidUrls(customFetch, customJSDOM, numUrls = settings.numberOfUrls) {
  ...
  async function extractUrlsFromPage(url) {
    try {
      ...
      if (!response.ok || !settings.allowedContentTypes.includes(response.headers.get("content-type")?.split(';')[0])) {
        return [];
      }
  ...
  async function crawl(startUrl) {
    ...
    if (now - lastAccessed < settings.crawlDelay) { // Use crawlDelay from settings
    ...
    if (await isValidUrl(startUrl)) {
      if (isNewDomain(startUrl, validUrls)) {
        ...
        const urlsFromDomain = Array.from(validUrls).filter(url => getDomain(url) === domain).length;
      ...
      if (validUrls.size < numUrls) {
        ...
        await Promise.all(
          prioritizedUrls.map(url => crawl(url))
        );
      }
    }
  }
