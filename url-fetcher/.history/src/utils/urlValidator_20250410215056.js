...
export async function isValidUrl(url) {
  ...
  if (settings.urlPatterns) {
    if (settings.urlPatterns.include && !settings.urlPatterns.include.some(pattern => pattern.test(url))) {
      return false;
    }
    if (settings.urlPatterns.exclude && settings.urlPatterns.exclude.some(pattern => pattern.test(url))) {
      return false;
    }
  }
